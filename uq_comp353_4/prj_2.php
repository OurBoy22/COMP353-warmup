<?php include 'config.php'; ?>

<!DOCTYPE html>
<header>
    <h1>Exercise 2</h1>
</header>

<link rel="stylesheet" type="text/css" href="styles.css">

<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Manage Personel</title>
    <script>
        function handleActionChange() {
        var action = document.getElementById("actionSelect").value;
        var formContent = document.getElementById("formContent");
        formContent.innerHTML = "";

        if (action === "create") {
            formContent.innerHTML = `
                <h3>Create Employee</h3>
                <label>First Name:</label> <input type="text" id="first_name" required><br>
                <label>Last Name:</label> <input type="text" id="last_name" required><br>
                <label>date of birth (YYYY-MM-DD):</label> <input type="date" id="dob" required><br>
                <label>SSN:</label> <input type="text" id="ssn" required><br>
                <label>Medical Card:</label> <input type="text" id="med_card" required><br>
                <label>Phone Number:</label> <input type="text" id="phone_num" required><br>
                <label>Email:</label> <input type="text" id="email" required><br>
                <label>Address: </label> <input type="text" id="address" required><br>
                <label>City:</label> <input type="text" id="city" required><br>
                <label>Province:</label> <input type="text" id="province" required><br>
                <label>Postal Code:</label> <input type="text" id="postal_code" required><br>
                <button type="button" onclick="createEmployee()">Submit</button>
                
                <h3>Create Contract</h3>
                <label>Location ID:</label> <input type="number" id="location_id" required><br>
                <label>Personel ID:</label> <input type="number" id="personel_id" required><br>
                <label>Start Date (YYYY-MM-DD):</label> <input type="date" id="start_date" required><br>
                <label>End Date (YYYY-MM-DD):</label> <input type="date" id="end_date" required><br>
                <label>Role:</label> <input type="text" id="role" required><br>
                <label>Mandate:</label> <input type="text" id="mandate" required><br>

                <button type="button" onclick="createContract()">Submit</button>
            `;
        } else if (action === "delete") {
            formContent.innerHTML = `
                <h3>Delete Personel</h3>
                <label>Personel ID:</label> <input type="number" id="personel_id" required><br>

                <button type="button" onclick="deletePersonel()">Delete</button>
            `;
        } else if (action === "edit") {
            formContent.innerHTML = `
                <h3>Edit Personel</h3>
                <h4>Enter the Personel ID of the person you want to edit:</h4>
                <label>Personel ID:</label> <input type="number" id="personel_id" required><br>
                <button type="button" onclick="getOneDataEmployee()">Get Data</button>

                <label>First Name:</label> <input type="text" id="new_first_name"><br>
                <label>Last Name:</label> <input type="text" id="new_last_name"><br>
                <label>date of birth (YYYY-MM-DD):</label> <input type="date" id="new_dob"><br>
                <label>SSN:</label> <input type="text" id="new_ssn"><br>
                <label>Medical Card:</label> <input type="text" id="new_med_card"><br>
                <label>Phone Number:</label> <input type="text" id="new_phone_num"><br>
                <label>Email:</label> <input type="text" id="new_email"><br>
                <label>Address: </label> <input type="text" id="new_address"><br>
                <label>City:</label> <input type="text" id="new_city"><br>
                <label>Province:</label> <input type="text" id="new_province"><br>
                <label>Postal Code:</label> <input type="text" id="new_postal_code"><br>
                <button type="button" onclick="updateEmployee()">Update</button>

                <h3>Edit Contract</h3>
                <h4>Enter the Contract ID of the contract you want to edit:</h4>
                <label>Contract ID:</label> <input type="number" id="contract_id" required><br>
                <button type="button" onclick="getOneDataContract()">Get Data</button>

                <label>Location ID:</label> <input type="number" id="new_location_id"><br>
                <label>Personel ID:</label> <input type="number" id="new_personel_id"><br>
                <label>Start Date (YYYY-MM-DD):</label> <input type="date" id="new_start_date"><br>
                <label>End Date (YYYY-MM-DD):</label> <input type="date" id="new_end_date"><br>
                <label>Role:</label> <input type="text" id="new_role"><br>
                <label>Mandate:</label> <input type="text" id="new_mandate"><br>
                <button type="button" onclick="updateContract()">Update</button>
                
            `;
        } else if (action === "display") {
            fetch('personel_handler.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                body: 'action=display'
            })
            .then(response => response.text())  // Get the response as plain text
            .then(data => {
                // Check if the response contains an error
                if (data.includes("Error")) {
                    // Handle error message
                    formContent.innerHTML = `<div class="error">${data}</div>`;  // Display the error
                } else {
                    // Handle success message
                    formContent.innerHTML = `<div class="success">${data}</div>`;  // Display the success message
                }
            })
            .catch(error => console.error('Error:', error));
        }
    }

    function deletePersonel(){
        var formData = new FormData();
        formData.append("action", "delete_personel");
        formData.append("personel_id", document.getElementById("personel_id").value);

        fetch('personel_handler.php', {
            method: 'POST',
            body: formData
        })
        .then(response => {
            console.log("Response URL:", response.url);  // Check if it's really location_handler.php
            return response.text();
        })
        .then(data => {
            console.log("Response Data:", data);  // Log the actual response to debug
            alert(data);
            // formContent.innerHTML = data;
        })
        .catch(error => console.error('Fetch Error:', error));
    }

    function createEmployee(){
        var formData = new FormData();
        formData.append("action", "create_employee");
        formData.append("first_name", document.getElementById("first_name").value);
        formData.append("last_name", document.getElementById("last_name").value);
        formData.append("dob", document.getElementById("dob").value);
        formData.append("ssn", document.getElementById("ssn").value);
        formData.append("med_card", document.getElementById("med_card").value);
        formData.append("phone_num", document.getElementById("phone_num").value);
        formData.append("email", document.getElementById("email").value);
        formData.append("address", document.getElementById("address").value);
        formData.append("city", document.getElementById("city").value);
        formData.append("province", document.getElementById("province").value);
        formData.append("postal_code", document.getElementById("postal_code").value);

        fetch('personel_handler.php', {
            method: 'POST',
            body: formData
        })
        .then(response => {
            console.log("Response URL:", response.url);  // Check if it's really location_handler.php
            return response.text();
        })
        .then(data => {
            console.log("Response Data:", data);  // Log the actual response to debug
            alert(data);
            // formContent.innerHTML = data;
        })
        .catch(error => console.error('Fetch Error:', error));
    }

    function createContract(){
        var formData = new FormData();
        formData.append("action", "create_contract");
        formData.append("location_id", document.getElementById("location_id").value);
        formData.append("personel_id", document.getElementById("personel_id").value);
        formData.append("start_date", document.getElementById("start_date").value);
        formData.append("end_date", document.getElementById("end_date").value);
        formData.append("role", document.getElementById("role").value);
        formData.append("mandate", document.getElementById("mandate").value);

        fetch('personel_handler.php', {
            method: 'POST',
            body: formData
        })
        .then(response => {
            console.log("Response URL:", response.url);  // Check if it's really location_handler.php
            return response.text();
        })
        .then(data => {
            console.log("Response Data:", data);  // Log the actual response to debug
            alert(data);
            // formContent.innerHTML = data;
        })
        .catch(error => console.error('Fetch Error:', error));

    }

    function updateEmployee(){
        var formData = new FormData();
        formData.append("action", "update_employee");
        formData.append("personel_id", document.getElementById("personel_id").value);
        if (document.getElementById("new_first_name").value) formData.append("new_first_name", document.getElementById("new_first_name").value);
        if (document.getElementById("new_last_name").value) formData.append("new_last_name", document.getElementById("new_last_name").value);
        if (document.getElementById("new_dob").value) formData.append("new_dob", document.getElementById("new_dob").value);
        if (document.getElementById("new_ssn").value) formData.append("new_ssn", document.getElementById("new_ssn").value);
        if (document.getElementById("new_med_card").value) formData.append("new_med_card", document.getElementById("new_med_card").value);
        if (document.getElementById("new_phone_num").value) formData.append("new_phone_num", document.getElementById("new_phone_num").value);
        if (document.getElementById("new_email").value) formData.append("new_email", document.getElementById("new_email").value);
        if (document.getElementById("new_address").value) formData.append("new_address", document.getElementById("new_address").value);
        if (document.getElementById("new_city").value) formData.append("new_city", document.getElementById("new_city").value);
        if (document.getElementById("new_province").value) formData.append("new_province", document.getElementById("new_province").value);
        if (document.getElementById("new_postal_code").value) formData.append("new_postal_code", document.getElementById("new_postal_code").value);

        fetch('personel_handler.php', {
            method: 'POST',
            body: formData
        })
        .then(response => {
            console.log("Response URL:", response.url);  // Check if it's really location_handler.php
            return response.text();
        })
        .then(data => {
            console.log("Response Data:", data);  // Log the actual response to debug
            alert(data);
            // formContent.innerHTML = data;
        })
        .catch(error => console.error('Fetch Error:', error));
    }

    function updateContract(){
        var formData = new FormData();
        formData.append("action", "update_contract");
        formData.append("contract_id", document.getElementById("contract_id").value);
        if (document.getElementById("new_location_id").value) formData.append("new_location_id", document.getElementById("new_location_id").value);
        if (document.getElementById("new_personel_id").value) formData.append("new_personel_id", document.getElementById("new_personel_id").value);
        if (document.getElementById("new_start_date").value) formData.append("new_start_date", document.getElementById("new_start_date").value);
        if (document.getElementById("new_end_date").value) formData.append("new_end_date", document.getElementById("new_end_date").value);
        if (document.getElementById("new_role").value) formData.append("new_role", document.getElementById("new_role").value);
        if (document.getElementById("new_mandate").value) formData.append("new_mandate", document.getElementById("new_mandate").value);

        fetch('personel_handler.php', {
            method: 'POST',
            body: formData
        })
        .then(response => {
            console.log("Response URL:", response.url);  // Check if it's really location_handler.php
            return response.text();
        })
        .then(data => {
            console.log("Response Data:", data);  // Log the actual response to debug
            alert(data);
            // formContent.innerHTML = data;
        })
        .catch(error => console.error('Fetch Error:', error));  
    }

    function getOneDataEmployee(){
        var formData = new FormData();
        formData.append("action", "select_one_employee");
        formData.append("personel_id", document.getElementById("personel_id").value);

        fetch('personel_handler.php', {
            method: 'POST',
            body: formData
        })
        .then(response => {
            console.log("Response URL:", response.url);  // Check if it's really location_handler.php
            return response.text();
        })
        .then(data => {
            console.log("Response Data:", data);  // Log the actual response to debug
            
            // Parse the JSON response
            var personel = JSON.parse(data);

        
            console.log(data);
            document.getElementById("new_first_name").value = personel.first_name;
            document.getElementById("new_last_name").value = personel.last_name;
            document.getElementById("new_dob").value = personel.dob;
            document.getElementById("new_ssn").value = personel.ssn;
            document.getElementById("new_med_card").value = personel.med_card;
            document.getElementById("new_phone_num").value = personel.phone_num;
            document.getElementById("new_email").value = personel.email;
            document.getElementById("new_address").value = personel.address;
            document.getElementById("new_city").value = personel.city;
            document.getElementById("new_province").value = personel.province;
            document.getElementById("new_postal_code").value = personel.postal_code;
        })
        .catch(error => console.error('Fetch Error:', error));
    }

    function getOneDataContract(){
        var formData = new FormData();
        formData.append("action", "select_one_contract");
        formData.append("contract_id", document.getElementById("contract_id").value);

        fetch('personel_handler.php', {
            method: 'POST',
            body: formData
        })
        .then(response => {
            console.log("Response URL:", response.url);  // Check if it's really location_handler.php
            return response.text();
        })
        .then(data => {
            console.log("Response Data:", data);  // Log the actual response to debug
            
            // Parse the JSON response
            var contract = JSON.parse(data);

        
            console.log(data);
            document.getElementById("new_location_id").value = contract.location_id;
            document.getElementById("new_personel_id").value = contract.personel_id;
            document.getElementById("new_start_date").value = contract.term_start_date;
            document.getElementById("new_end_date").value = contract.term_end_date;
            document.getElementById("new_role").value = contract.role;
            document.getElementById("new_mandate").value = contract.mandate;
        })
        .catch(error => console.error('Fetch Error:', error));
     
    }

    
    function submitForm() {
        var action = document.getElementById("actionSelect").value;
        var formData = new FormData();
        formData.append("action", action);

        if (action === "create") {
            formData.append("location_id", document.getElementById("location_id").value);
            formData.append("name", document.getElementById("name").value);
            formData.append("type", document.getElementById("type").value);
            formData.append("phone", document.getElementById("phone").value);
            formData.append("max_capacity", document.getElementById("max_capacity").value);
            formData.append("website", document.getElementById("website").value);
            
            formData.append("address_id", document.getElementById("address_id").value);
            formData.append("address", document.getElementById("address").value);
            formData.append("city", document.getElementById("city").value);
            formData.append("province", document.getElementById("province").value);
            formData.append("postal_code", document.getElementById("postal_code").value);
        } else if (action === "delete") {
            formData.append("location_id", document.getElementById("location_id").value);
        } else if (action === "edit") {
            formData.append("location_id", document.getElementById("location_id").value);
            if (document.getElementById("new_name").value) formData.append("new_name", document.getElementById("new_name").value);
            if (document.getElementById("new_type").value) formData.append("new_type", document.getElementById("new_type").value);
            if (document.getElementById("new_phone").value) formData.append("new_phone", document.getElementById("new_phone").value);
            if (document.getElementById("new_max_capacity").value) formData.append("new_max_capacity", document.getElementById("new_max_capacity").value);
            if (document.getElementById("new_website").value) formData.append("new_website", document.getElementById("new_website").value);
            
            if (document.getElementById("new_address").value) formData.append("new_address", document.getElementById("new_address").value);
            if (document.getElementById("new_city").value) formData.append("new_city", document.getElementById("new_city").value);
            if (document.getElementById("new_province").value) formData.append("new_province", document.getElementById("new_province").value);
            if (document.getElementById("new_postal_code").value) formData.append("new_postal_code", document.getElementById("new_postal_code").value);
        }

        fetch('location_handler.php', {
            method: 'POST',
            body: formData
        })
        .then(response => {
            console.log("Response URL:", response.url);  // Check if it's really location_handler.php
            return response.text();
        })
        .then(data => {
            console.log("Response Data:", data);  // Log the actual response to debug
            alert(data);
            if (data.includes("<html")) {
                console.error("Unexpected HTML returned, check PHP output!");
            }
            // formContent.innerHTML = data;
        })
        .catch(error => console.error('Fetch Error:', error));
            }
    </script>
</head>
<body>
    <div class="container">
        <h2>Manage Locations</h2>
        <p>Select an action:</p>
        <form action="location_handler.php" method="GET">
            <label for="actionSelect">Choose an action:</label>
            <select id="actionSelect" name="action" onchange="handleActionChange()">
                <option value="">--Select--</option>
                <option value="create">Create Employee / Contract</option>
                <option value="edit">Edit Employee / Contract</option>
                <option value="delete">Delete Employee / Contract</option>
                <option value="display">Display Employees / Contracts</option>
            </select>
            <br><br>
            <div id="formContent"></div>
            <div id="result"></div>
        </form>
    </div>
</body>

<?php include 'footer.php'; ?>
</html>
